...
export const api = {
  ...
  matchJob: async (payload: { user_id?: string; job_id: string; history_id?: string; ignored_skill_names?: string[]; added_from_missing_skills?: Array<{ skill_id: string; skill_name: string }>; persist_history?: boolean }) => {
    const body = { ...payload, user_id: payload.user_id ?? (await getUserIdOrThrow()) };
