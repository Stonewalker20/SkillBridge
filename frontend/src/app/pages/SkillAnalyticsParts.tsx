...
export function AnalyticsSection({ id, title, description, actions, children, className, compact = false }: AnalyticsSectionProps) {
  return (
    <Card id={id} className={cn("relative overflow-hidden border-slate-200 bg-white/90 dark:border-slate-800 dark:bg-slate-900/85", compact ? "p-4" : "p-5", className)}>
      ...
      <div className={cn("relative z-10 flex items-start justify-between gap-4", compact ? "mb-3" : "mb-4")}>
...
export function AnalyticsMetric({ icon: Icon, label, value, caption, toneClass }: AnalyticsMetricProps) {
  return (
    <Card className="relative overflow-hidden border-slate-200 bg-white/90 p-4 dark:border-slate-800 dark:bg-slate-900/85">
      ...
      <div className="relative flex items-start gap-3">
        <div className={cn("rounded-2xl p-3 shadow-lg shadow-slate-900/10", toneClass)}>
