...
\newcommand{\SafeImageOrPlaceholder}[2]{%
  \IfFileExists{#1}{%
    \fbox{\includegraphics[width=#2]{#1}}%
  }{%
    \fbox{\parbox[c][2.25in][c]{#2}{\centering Missing image placeholder\\[0.35em]\texttt{\detokenize{#1}}}}%
  }%
}
...
\newcommand{\ImagePage}[3]{
  ...
  \begin{figure}[H]
    \centering
    \SafeImageOrPlaceholder{#2}{0.95\linewidth}
    \caption{#3}
  \end{figure}
}
...
\newcommand{\MultiImagePageTwo}[6]{
  ...
  \begin{figure}[H]
    ...
    \begin{tabularx}{\linewidth}{@{}X X@{}}
      ...
      \SafeImageOrPlaceholder{#2}{0.95\linewidth} &
      \centering
      \SafeImageOrPlaceholder{#4}{0.95\linewidth} \\
      \small #3 & \small #5 \\
    \end{tabularx}
  \end{figure}
}
...
\newcommand{\MultiImagePageFour}[9]{
  ...
  \begin{figure}[H]
    ...
    \begin{tabularx}{\linewidth}{@{}X X@{}}
      ...
      \\
      \centering
      \SafeImageOrPlaceholder{#6}{0.95\linewidth} &
      \centering
      \SafeImageOrPlaceholder{#8}{0.95\linewidth} \\
      \small #7 & \small #9 \\
    \end{tabularx}
  \end{figure}
}
...
% =========================================================
% WEEK 6 (PR6) -- MOST RECENT
% =========================================================
\WeekReport{6}{March 11 -- March 17, 2026}{
Week 6 focused on role intelligence and taxonomy maintenance. SkillBridge now supports computing role-specific skill weights from approved job postings, maintaining skill aliases as part of the shared taxonomy, and storing explicit relationships between skills so the analytics and role-matching layers can reason over a cleaner graph.
}{
Justin Elia & Validation support for role aggregation evidence & \\
Spencer Roeren & Report review and taxonomy UI alignment support & \\
Cordell Stonecipher & Implemented UC 4.3 and UC 4.4 endpoints + aggregation logic + testing + report updates & (Artifacts: roles/taxonomy routes + tests + report updates) \\
}
...
\begin{itemize}
  ...
  \item \textbf{UC 4.4 -- Maintain Skill Aliases}
  \item \textbf{UC 4.4 -- Maintain Skill Relationships}
\end{itemize}
...
\noindent
The PR6 screenshots have not been exported into the repository yet. The boxes below are compile-safe placeholders for the final report draft.

\begin{figure}[H]
  \centering
  \begin{tabular}{cc}
    \fbox{\parbox[c][2.1in][c]{0.42\linewidth}{\centering UC 4.3 Placeholder\\Role weight computation response\\and role\_skill\_weights collection view}} &
    \fbox{\parbox[c][2.1in][c]{0.42\linewidth}{\centering UC 4.4 Placeholder\\Alias update request\\and updated skills document}} \\
    \small Planned evidence: role aggregation workflow & \small Planned evidence: alias maintenance workflow \\
    \\[0.75em]
    \fbox{\parbox[c][2.1in][c]{0.42\linewidth}{\centering UC 4.4 Placeholder\\Create relation request\\and skill\_relations collection view}} &
    \fbox{\parbox[c][2.1in][c]{0.42\linewidth}{\centering PR6 Use Case Diagram Placeholder\\Highlight UC 4.3 and UC 4.4}} \\
    \small Planned evidence: taxonomy relationships workflow & \small Planned evidence: highlighted use case diagram \\
  \end{tabular}
  \caption{PR6 implementation evidence placeholders for UC 4.3 and UC 4.4.}
\end{figure}

\subsection*{Week 6 -- Database Queries Used (MongoDB Equivalent to SQL)}
\addcontentsline{toc}{subsection}{Week 6 -- Database Queries Used (MongoDB Equivalent to SQL)}

\noindent
\textbf{UC 4.3 -- Aggregate Postings by Role and Compute Skill Weights}
\begin{verbatim}
db.jobs.find(
...
db.role_skill_weights.updateOne(
  ...
  { $set: {
      ...
      weights: [
        { skill_id: "<skill_id>", skill_name: "<skill_name>", weight: 0.75 }
      ]
  }},
  { upsert: true }
)
...
\noindent
\textbf{UC 4.4 -- Maintain Skill Aliases}
\begin{verbatim}
db.skills.updateOne(
...
\noindent
\textbf{UC 4.4 -- Maintain Skill Relationships}
\begin{verbatim}
...
\noindent
Video link: To be added for PR6 delivery.

% =========================================================
% WEEK 5 (PR5)
% =========================================================
\WeekReport{5}{March 4 -- March 10, 2026}{
Week 5 focused on persistence workflows that connect extracted artifacts back into the user's long-term profile. SkillBridge now supports saving evidence directly against skills and projects, promoting confirmed resume-derived skills into reusable dashboard entities, and tagging job postings with target roles so downstream role analytics can group them correctly.
}{
Justin Elia & Validation support for evidence and promotion flows & \\
Spencer Roeren & Report review and UI alignment support & \\
Cordell Stonecipher & Implemented UC 1.3, UC 3.4, and UC 4.2 endpoints + persistence logic + testing + report updates & (Artifacts: evidence/resume/jobs routes + tests + report updates) \\
}

\subsection*{Week 5 -- Implemented Use Cases}
\addcontentsline{toc}{subsection}{Week 5 -- Implemented Use Cases}
\begin{itemize}
  \item \textbf{UC 1.3 -- Add Evidence Artifact and Associate It with a Skill/Project}
  \item \textbf{UC 3.4 -- Save Confirmed Resume-Derived Skills/Projects into Dashboard Entities}
...
\noindent
The PR5 screenshots have not been exported into the repository yet. The boxes below are compile-safe placeholders for the final report draft.

\begin{figure}[H]
  \centering
  \begin{tabular}{cc}
    \fbox{\parbox[c][2.1in][c]{0.42\linewidth}{\centering UC 1.3 Placeholder\\Saved evidence API response\\and evidence collection view}} &
    \fbox{\parbox[c][2.1in][c]{0.42\linewidth}{\centering UC 3.4 Placeholder\\Resume promotion response\\and linked project/evidence records}} \\
    \small Planned evidence: evidence persistence workflow & \small Planned evidence: resume promotion workflow \\
    \\[0.75em]
    \fbox{\parbox[c][2.1in][c]{0.42\linewidth}{\centering UC 4.2 Placeholder\\Tagged job response\\and updated job role\_ids field}} &
    \fbox{\parbox[c][2.1in][c]{0.42\linewidth}{\centering PR5 Use Case Diagram Placeholder\\Highlight UC 1.3, UC 3.4, and UC 4.2}} \\
    \small Planned evidence: job role-tagging workflow & \small Planned evidence: highlighted use case diagram \\
  \end{tabular}
  \caption{PR5 implementation evidence placeholders for UC 1.3, UC 3.4, and UC 4.2.}
\end{figure}

\subsection*{Week 5 -- Database Queries Used (MongoDB Equivalent to SQL)}
\addcontentsline{toc}{subsection}{Week 5 -- Database Queries Used (MongoDB Equivalent to SQL)}

\noindent
\textbf{UC 1.3 -- Add Evidence Artifact and Associate It with a Skill/Project}
\begin{verbatim}
...
\noindent
\textbf{UC 3.4 -- Save Confirmed Resume-Derived Skills/Projects into Dashboard Entities}
\begin{verbatim}
db.projects.insertOne({
  user_id: ObjectId("<user_id>"),
  title: "Resume Snapshot <snapshot_id_prefix>",
...
db.project_skill_links.updateOne(
  { project_id: ObjectId("<project_id>"), skill_id: ObjectId("<skill_id>") },
  { $setOnInsert: {
      project_id: ObjectId("<project_id>"),
...
db.evidence.insertOne({
  ...
  type: "resume",
  title: "<skill_name>",
  source: "resume_snapshot:<snapshot_id>",
  project_id: ObjectId("<project_id>"),
  skill_ids: [ObjectId("<skill_id>")],
  created_at: new Date(),
  updated_at: new Date()
...
\noindent
Video link: To be added for PR5 delivery.

% =========================================================
% WEEK 4 (PR4)
% =========================================================
\WeekReport{4}{February 26 -- March 3, 2026}{
Week 4 focused on extending SkillBridge beyond ingestion and extraction into user-facing project organization, personalized dashboard aggregation, and admin moderation workflows. The backend now supports linking confirmed skills to specific portfolio projects, computing dashboard summaries from multiple user collections, and moderating community-submitted job postings through an approval or rejection flow.
}{
Justin Elia & Validation support for project and moderation flows & \\
Spencer Roeren & Report review and UI alignment support & \\
Cordell Stonecipher & Implemented UC 1.2, UC 1.4, and UC 4.1 endpoints + aggregation logic + testing & (Artifacts: API routes + tests + report updates) \\
}

\subsection*{Week 4 -- Implemented Use Cases}
\addcontentsline{toc}{subsection}{Week 4 -- Implemented Use Cases}
\begin{itemize}
  \item \textbf{UC 1.2 -- Link Skill to Project}
  \item \textbf{UC 1.4 -- Dashboard Summary (User-Specific)}
  \item \textbf{UC 4.1 -- Moderate Job Postings}
\end{itemize}
...
\noindent
The PR4 screenshots have not been exported into the repository yet. The boxes below are compile-safe placeholders for the final report draft.

\begin{figure}[H]
  \centering
  \begin{tabular}{cc}
    \fbox{\parbox[c][2.1in][c]{0.42\linewidth}{\centering UC 1.2 Placeholder\\Project skill link API response\\and project\_skill\_links collection view}} &
    \fbox{\parbox[c][2.1in][c]{0.42\linewidth}{\centering UC 1.4 Placeholder\\Dashboard summary API response\\and dashboard metric cards}} \\
    \small Planned evidence: project-skill linking & \small Planned evidence: dashboard summary aggregation \\
    \\[0.75em]
    \fbox{\parbox[c][2.1in][c]{0.42\linewidth}{\centering UC 4.1 Placeholder\\Pending jobs list, moderation PATCH\\and updated moderation status}} &
    \fbox{\parbox[c][2.1in][c]{0.42\linewidth}{\centering PR4 Use Case Diagram Placeholder\\Highlight UC 1.2, UC 1.4, and UC 4.1}} \\
    \small Planned evidence: moderation workflow & \small Planned evidence: highlighted use case diagram \\
  \end{tabular}
  \caption{PR4 implementation evidence placeholders for UC 1.2, UC 1.4, and UC 4.1.}
\end{figure}

\subsection*{Week 4 -- Database Queries Used (MongoDB Equivalent to SQL)}
\addcontentsline{toc}{subsection}{Week 4 -- Database Queries Used (MongoDB Equivalent to SQL)}

\noindent
\textbf{UC 1.2 -- Link Skill to Project}
\begin{verbatim}
db.project_skill_links.findOne({
  project_id: ObjectId("<project_id>"),
  skill_id: ObjectId("<skill_id>")
})

db.project_skill_links.insertOne({
  project_id: ObjectId("<project_id>"),
  skill_id: ObjectId("<skill_id>"),
  created_at: new Date()
})
...
\begin{verbatim}
db.projects.find(
  { user_id: { $in: [ObjectId("<user_id>"), "<user_id>"] } },
  { title: 1, description: 1, created_at: 1 }
).sort({ created_at: -1 }).limit(10)

db.evidence.aggregate([
  { $match: { user_id: { $in: [ObjectId("<user_id>"), "<user_id>"] } } },
  { $unwind: "$skill_ids" },
...
db.resume_skill_confirmations.aggregate([
  { $match: { user_id: { $in: [ObjectId("<user_id>"), "<user_id>"] }, resume_snapshot_id: null } },
  { $unwind: "$confirmed" },
...
\noindent
\textbf{UC 4.1 -- Moderate Job Postings}
\begin{verbatim}
db.jobs.find({ moderation_status: "pending" }).sort({ created_at: -1 })

db.jobs.updateOne(
  { _id: ObjectId("<job_id>") },
  { $set: {
      moderation_status: "rejected",
      moderation_reason: "Rejected by reviewer",
      updated_at: new Date()
  }}
)
\end{verbatim}
...
\noindent
Video link: To be added for PR4 delivery.
...
\noindent
Video link: \url{https://drive.google.com/file/d/1VNMVQtzIe4HL9UoQhuhnphgiitfG7b9m/view?usp=drive_link}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{tikzpicture}
            \node[anchor=south west, inner sep=0] (image) at (0,0) {\SafeImageOrPlaceholder{images/Dashboard.png}{\linewidth}};
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{tikzpicture}
            \node[anchor=south west, inner sep=0] (image) at (0,0) {\SafeImageOrPlaceholder{images/Skills.png}{\linewidth}};
            \begin{scope}[x={(image.south east)},y={(image.north west)}]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{tikzpicture}
            \node[anchor=south west, inner sep=0] (image) at (0,0) {\SafeImageOrPlaceholder{images/Evidence.png}{\linewidth}};
            \begin{scope}[x={(image.south east)},y={(image.north west)}]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{tikzpicture}
            \node[anchor=south west, inner sep=0] (image) at (0,0) {\SafeImageOrPlaceholder{images/Job Browser.png}{\linewidth}};
            \begin{scope}[x={(image.south east)},y={(image.north west)}]
