...
\begin{frame}{Database ERD}
  ...
  \begin{tikzpicture}[
    ...
    \node[panel, fill=sbsky!10] at (2.7,3.85) [minimum width=4.10cm, minimum height=4.20cm] {};
    \node[label] at (2.7,5.55) {Identity + Support};
    \node[panel, fill=sbteal!8] at (8.4,3.85) [minimum width=9.10cm, minimum height=4.20cm] {};
    \node[label] at (8.4,5.55) {Portfolio + Skill Graph};
    \node[panel, fill=sbgold!10] at (14.7,2.55) [minimum width=5.30cm, minimum height=6.90cm] {};
    \node[label] at (14.7,5.55) {Resume + Matching};

    \node[box, fill=oublack] (users) at (8.4,7.0) {USERS};

    \node[box, fill=sbsky!85!oublack] (sessions) at (2.7,4.8) {SESSIONS};
    \node[box, fill=sbviolet!75!oublack] (help) at (2.7,2.85) {HELP\\REQUESTS};

    \node[box, fill=sbcoral!88!black] (projects) at (5.7,4.8) {PROJECTS};
    \node[box, fill=sbcoral] (links) at (8.4,4.8) {PROJECT\_SKILL\\LINKS};
    \node[box, fill=sbteal] (skills) at (11.1,4.8) {SKILLS};
    \node[box, fill=sbteal!80!black] (evidence) at (5.7,2.85) {EVIDENCE};
    \node[box, fill=sbviolet] (confirm) at (11.1,2.85) {RESUME\_SKILL\\CONFIRMATIONS};

    \node[box, fill=sbsky!65!sbviolet] (resume) at (14.6,4.8) {RESUME\\SNAPSHOTS};
    \node[box, fill=sbcoral!82!sbviolet] (jobs) at (14.6,2.85) {JOB\_INGESTS\\+ JOBS};
    \node[box, fill=sbgold!85!black] (matchruns) at (14.2,0.90) {JOB\_MATCH\\RUNS};
    \node[box, fill=sbgold!70!sbteal] (tailored) at (17.6,0.90) {TAILORED\\RESUMES};
    ...
    \node[note] at (9.4,-0.35) {Users supply proof, resumes, and jobs; skills connect the portfolio side to matching outcomes.};
  \end{tikzpicture}
  }
\end{frame}
