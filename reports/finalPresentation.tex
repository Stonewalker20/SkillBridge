...
\newcommand{\oaklandbrandmark}{%
  \IfFileExists{\oaklandlogopath}{%
    \includegraphics[height=0.60cm]{\oaklandlogopath}%
  }{%
...
\newcommand{\oaklandbrandbadge}{%
  \raisebox{0.06cm}{\tikz[baseline=(box.base)]\node[rounded corners=4pt, fill=oustone, inner xsep=4pt, inner ysep=1.5pt] (box) {\oaklandbrandmark};}%
}
...
\setbeamertemplate{title page}{%
  ...
  \begin{minipage}[t][0.77\textheight][t]{\textwidth}
    {\color{white}\fontsize{21}{25}\selectfont\bfseries\inserttitle\par}
    \vspace{0.45em}
    ...
    \IfFileExists{\oaklandwordmarkpath}{%
      \begin{center}
        \includegraphics[width=0.28\linewidth]{\oaklandwordmarkpath}
      \end{center}
    }{%
      \begin{center}
        {\color{white!85}\large Oakland University}
      \end{center}
    }%
  \end{minipage}
}
...
\begin{frame}{Users and Core Flows}
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    {\small\bfseries User Types}\\[0.35em]
    \includegraphics[width=\linewidth,height=0.32\textheight,keepaspectratio]{../use_case_standard_user.png}
    \vspace{0.55em}
    \includegraphics[width=\linewidth,height=0.32\textheight,keepaspectratio]{../use_case_admin_user.png}
    \column{0.48\textwidth}
    {\small\bfseries Core Pipelines}\\[0.35em]
    \includegraphics[width=\linewidth,height=0.30\textheight,keepaspectratio]{../Frontend_evidence_component_flowchart.png}
    \vspace{0.55em}
    \includegraphics[width=\linewidth,height=0.30\textheight,keepaspectratio]{../job_match_analysis_pipeline_flowchart.png}
  \end{columns}
\end{frame}
...
\begin{frame}{Database ERD}
  ...
  \begin{tikzpicture}[
    ...
    \node[box, fill=oublack] (users) at (4.6,6.5) {USERS};
    \node[box, fill=sbsky!85!oublack] (sessions) at (0.7,5.1) {SESSIONS};
    \node[box, fill=sbteal] (skills) at (8.4,5.1) {SKILLS};
    \node[box, fill=sbcoral!88!black] (projects) at (0.7,3.05) {PROJECTS};
    \node[box, fill=sbcoral] (links) at (3.25,3.05) {PROJECT\_SKILL\\LINKS};
    \node[box, fill=sbteal!80!black] (evidence) at (5.95,3.05) {EVIDENCE};
    \node[box, fill=sbviolet] (confirm) at (1.55,0.85) {RESUME\_SKILL\\CONFIRMATIONS};
    \node[box, fill=sbsky!65!sbviolet] (resume) at (4.55,0.85) {RESUME\\SNAPSHOTS};
    \node[box, fill=sbcoral!82!sbviolet] (jobs) at (7.65,0.85) {JOB\_INGESTS\\+ JOBS};
    \node[box, fill=sbgold!85!black] (matchruns) at (3.15,-1.35) {JOB\_MATCH\\RUNS};
    \node[box, fill=sbgold!70!sbteal] (tailored) at (6.05,-1.35) {TAILORED\\RESUMES};
    \node[box, fill=sbviolet!75!oublack] (help) at (10.65,5.1) {HELP\\REQUESTS};
    ...
    \node[note] at (4.9,5.82) {User is the central owner across every workflow};
    \node[note] at (5.25,2.3) {Skills connect proof, projects, and confirmations};
    \node[note] at (5.2,-2.0) {Job analysis feeds match history and tailored resume output};
  \end{tikzpicture}
  }
\end{frame}
...
\begin{frame}[fragile,shrink=5]{Skills Page}
  \begin{columns}[T,totalwidth=\textwidth]
    \column{0.28\textwidth}
    \vspace{-0.45em}
    \begin{block}{Page Coverage}
...
\begin{frame}[fragile,shrink=5]{Resume Workflow Pages}
  \begin{columns}[T,totalwidth=\textwidth]
    \column{0.28\textwidth}
    \vspace{-0.45em}
    \begin{block}{Page Coverage}
...
\begin{frame}[fragile,shrink=5]{Admin Job Review Page}
  \begin{columns}[T,totalwidth=\textwidth]
    \column{0.28\textwidth}
    \vspace{-0.45em}
    \begin{block}{Page Coverage}
