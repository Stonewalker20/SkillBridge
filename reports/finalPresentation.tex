...
\newcommand{\oaklandbrandmark}{%
  \IfFileExists{\oaklandlogopath}{%
    \includegraphics[height=0.40cm]{\oaklandlogopath}%
  }{%
...
\newcommand{\oaklandbrandbadge}{%
  \raisebox{0.18cm}{\tikz[baseline=(box.base)]\node[rounded corners=4pt, fill=oustone, inner xsep=4pt, inner ysep=1pt] (box) {\oaklandbrandmark};}%
}
...
\setbeamertemplate{background canvas}{%
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \fill[oustone] (current page.south west) rectangle ([yshift=0.56cm]current page.south east);
    ...
    \draw[ouline,line width=0.4pt] ([yshift=0.56cm]current page.south west) -- ([yshift=0.56cm]current page.south east);
  \end{tikzpicture}
}
...
\setbeamertemplate{footline}{%
  \leavevmode%
  \hbox{%
    \begin{beamercolorbox}[wd=\paperwidth,ht=0.82cm,dp=0.24cm,leftskip=0.38cm,rightskip=0.3cm]{footline}
      \begin{minipage}[c][0.82cm][c]{0.70\paperwidth}
        {\scriptsize\bfseries Oakland University}\hspace{0.55em}{\scriptsize School of Engineering and Computer Science}
      \end{minipage}%
      \begin{minipage}[c][0.82cm][c]{0.14\paperwidth}
        \raggedleft{\scriptsize\insertframenumber/\inserttotalframenumber}
      \end{minipage}%
      \begin{minipage}[c][0.82cm][c]{0.12\paperwidth}
...
\begin{frame}{Database ERD}
  ...
  \begin{tikzpicture}[
    ...
    link/.style={line width=1.05pt, draw=sbnavy!82, rounded corners=6pt},
    note/.style={font=\scriptsize\color{sbmuted}, align=center}
  ]
    \node[box, fill=oublack] (users) at (5.2,6.45) {USERS};
    \node[box, fill=sbsky!85!oublack] (sessions) at (1.2,5.15) {SESSIONS};
    \node[box, fill=sbviolet!75!oublack] (help) at (9.2,5.15) {HELP\\REQUESTS};
    \node[box, fill=sbcoral!88!black] (projects) at (1.2,3.6) {PROJECTS};
    \node[box, fill=sbcoral] (links) at (3.55,3.6) {PROJECT\_SKILL\\LINKS};
    \node[box, fill=sbteal] (skills) at (5.9,3.6) {SKILLS};
    \node[box, fill=sbteal!80!black] (evidence) at (8.25,3.6) {EVIDENCE};
    \node[box, fill=sbviolet] (confirm) at (1.9,1.75) {RESUME\_SKILL\\CONFIRMATIONS};
    \node[box, fill=sbsky!65!sbviolet] (resume) at (4.95,1.75) {RESUME\\SNAPSHOTS};
    \node[box, fill=sbcoral!82!sbviolet] (jobs) at (8.0,1.75) {JOB\_INGESTS\\+ JOBS};
    \node[box, fill=sbgold!85!black] (matchruns) at (5.55,-0.05) {JOB\_MATCH\\RUNS};
    \node[box, fill=sbgold!70!sbteal] (tailored) at (8.55,-0.05) {TAILORED\\RESUMES};

    \coordinate (userbus) at (5.2,5.55);
    \coordinate (skillbus) at (5.9,2.55);
    \coordinate (jobbus) at (8.0,0.8);

    \draw[link] (users.south) -- (userbus);
    \draw[link] (userbus) -| (sessions.east);
    \draw[link] (userbus) -| (help.west);
    \draw[link] (userbus) -| (projects.north);
    \draw[link] (userbus) -| (skills.north);
    \draw[link] (userbus) -| (evidence.north);
    \draw[link] (userbus) -| (confirm.north);
    \draw[link] (userbus) -| (resume.north);
    \draw[link] (userbus) -| (jobs.north);

    \draw[link] (projects.east) -- (links.west);
    \draw[link] (links.east) -- (skills.west);
    \draw[link] (evidence.west) -- (skills.east);
    \draw[link] (confirm.east) -| (skillbus) -| (skills.south);
    \draw[link] (jobs.south) -- (jobbus);
    \draw[link] (jobbus) -| (matchruns.east);
    \draw[link] (jobbus) -| (tailored.north);
    \draw[link] (resume.south) |- (tailored.west);

    \node[note] at (7.0,-0.95) {Job ingest powers match history and tailored resume output};
  \end{tikzpicture}
  }
\end{frame}
